...
const STAGES = ["PICKED_UP", "IN_TRANSIT", "DELIVERED"];
...
export default function DriverTracking() {
  ...
  return (
    <StepShell eyebrow="LIVE NAVIGATION" title="En route to Buyer B" step={4} totalSteps={5} backHref="/driver/pickup">
      ...
      <div className="status-list">
        {STAGES.map((s, i) => (
          <div key={s} className={"status-row " + (i < idx ? "done" : i === idx ? "current" : "")}>{s.replaceAll("_", " ")}</div>
        ))}
      </div>
      <div className="step-actions">
        <button className="primary" disabled={idx >= STAGES.length - 1} onClick={advance}>
          {idx >= STAGES.length - 1 ? "Delivered" : "Advance"}
        </button>
      </div>
    </StepShell>
  );
